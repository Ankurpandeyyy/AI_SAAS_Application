...
export const resumeReview = async (req, res) => {
    try {
        ...
        //checking size of Resume
        if(resume.size > 5 * 1024 * 1024){
            return res.json({success: false, message: "Resume File size exceeds allowed size (5MB) ."})
        }
         
        //convert resume file in to data buffer
        const dataBuffer = fs.readFileSync(resume.path)
        const pdfData = await pdf(dataBuffer)

        const prompt = `Review the following resume and provide constructive
        feedback on its strengths, weaknesses, and areas for improvement also analyze it is
        ATS friendly resume or not .
        Resume Content:\n\n${pdfData.text}`
